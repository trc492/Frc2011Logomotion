...
#define ANALOGINF_USER_MASK     0x0000ffff
#define ANALOGINF_INVERSE       0x00000001
#define ANALOGINF_CALIBRATING   0x80000000
 ...
 */
class AnalogIn: public AnalogChannel
{
private:
    UINT32          m_slot;
    UINT32          m_channel;
...
public:
     ...
     * @param flags Specifies the flag value. For example, set the INVERSE
     *        flag will reverse the sense of high and low zones.
    ...
    {
        TLevel(INIT);
        TEnterMsg(("Slot=%d,Channel=%d,ThresholdLo=%d,ThresholdHi=%d,flags=%x,"
                   "notify=%p",
                   slot, channel, thresholdLo, thresholdHi, flags, notify));

        m_thresholdLo = thresholdLo;
        m_thresholdHi = thresholdHi;
        m_flagsAIn = flags & ANALOGINF_USER_MASK;
        ...
        )
    {
        TLevel(API);
        TEnter();
        TExitMsg(("=%d", (m_flagsAIn & ANALOGINF_CALIBRATING) != 0));
        return (m_flagsAIn & ANALOGINF_CALIBRATING) != 0;
    }   //IsCalibrating
        ...
        )
    {
        ...
        if (fStart)
        {
           m_flagsAIn |= ANALOGINF_CALIBRATING;
    ...
    /**
     * This function is called by the robot loop to check and process AnalogIn
     * channel triggers. It reads the value of an analog channel and compares
        ...
        )
    {
        ...
        else
        {
            ...
            {
                zone = (m_flagsAIn & ANALOGINF_INVERSE)?
